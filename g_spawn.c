...
void ED_CallSpawn (edict_t *ent)
{
	...
	if (!strcmp(ent->classname, "hmg_mod_colling")
		||!strcmp(ent->classname, "hmg_mod_cooling")
		|| !strcmp(ent->classname, "pistol_mod_reload")
		|| !strcmp(ent->classname, "pistol_mod_damage")
		)
...
static vec3_t spawnvecs_stdm5[] = {
	{ -40, 178, 0 },		//0	//#15	//stdm5 jump pad speed
	{ -40, 358, 0 },		//1	//#16	//stdm5 jump pad speed
	{ -45, 255, 0 },		//2	//#21	//stdm5 jump pad speed 
	{ -45, 285, 0 },		//3	//#22	//stdm5 jump pad speed	
	{ -90, 270, 0 },		//4	//#23	//stdm5	jump pad angle
	//weps to armour
	{ 800, 592, 1360 },		//5	//tommy top			//item_armor_jacket_heavy
	{ 872, 408, 1360 },		//6	//flametank top		//item_armor_helmet_heavy
	{ 728, 408, 1360 },		//7	//flametank top		//item_armor_legs_heavy
	{ 690, 1872, 146 },		//8	//hmg				//item_armor_legs_heavy
	{ 912, 1872, 146 },		//9	//hmg				//item_armor_helmet_heavy
	{ 1712, -48, 400 },		//10//shotty leg		//item_armor_helmet
	{ -112, -48, 400 },		//11//shotty helmet		//item_armor_legs
	{ 434, 484, 208 },		//12//rl mid			//pistol_mod_rof
	{ 1170, 484, 208 },		//13//rl mid			//pistol_mod_rof
	//items to health in air
	{ 1224, -56, 16 },		//14//cylinder			//item_armor_helmet
	{ 376, -56, 16 },		//15//shotty helmet		//item_armor_legs
	{ -64, -48, 400 },		//16//shoty shell		//pistol_mod_rof
	{ 1664, -48, 400 },		//17//shotty shell		//pistol_mod_rof
	//floating armour
	{ 800, 736, 208 },		//18//rl mid			// 792 8 528
	//mega healtyh
	{ 800, -56, 16 },		//19//flamegun			//800 512 440	
};

static vec3_t spawnvecs_2v2hypo[] = {
	{ -736, 984, - 144 },	//0	//flamer on ledge	//heavy leg
	{ -1400, 384, 8 },		//1	//hmg				//heavy body
	{ -2104, -1000, 16 },	//2	//GL going to HMG	//heavy leg
	{ -2048, 1000, -136 },	//3	//RL				//heavy head
	{ -832, -248, -544 },	//4	//ammo RL			//heavy body
	{ -952, -248, -544 },	//5	//ammo GL			//heavy head
};
//// HYPOV8_END
...
void SpawnEntities (char *mapname, char *entities, char *spawnpoint)
{
	...
	if (kpded2)
	{
		...
		dlindex("pics/mmod/gr_score.tga"); //hypov8
		if (teamplay->value)
		{
		...
		if (ent != g_edicts)
		{
			...
			if (enable_bots && !Q_stricmp(level.mapname, "2v2dm_hypo")){ //add hypov8
				if (VectorCompare(ent->s.origin, spawnvecs_2v2hypo[0]))
					...
					ent->classname = "item_armor_jacket_heavy";//heavy body
				else if (VectorCompare(ent->s.origin, spawnvecs_2v2hypo[2]))
					ent->classname = "item_armor_legs_heavy";//heavy leg
				else if (VectorCompare(ent->s.origin, spawnvecs_2v2hypo[3]))
					ent->classname = "item_armor_helmet_heavy";//heavy head
				else if (VectorCompare(ent->s.origin, spawnvecs_2v2hypo[4]))
					ent->classname = "item_armor_jacket_heavy";//heavy body
			...
			if (!Q_stricmp(level.mapname, "stdm5")) //add hypov8
			{
				...
				{
					if (VectorCompare(ent->s.angles, spawnvecs_stdm5[0])){
						ent->speed = 90.0f;
						ent->s.angles[1] = 173.0f;
					}
					else if (VectorCompare(ent->s.angles, spawnvecs_stdm5[1])){
						ent->speed = 90.0f;
						ent->s.angles[1] = 7.0f;
					}
					else if (VectorCompare(ent->s.angles, spawnvecs_stdm5[2]))
						ent->speed = 105.0f;
					else if (VectorCompare(ent->s.angles, spawnvecs_stdm5[3]))
						ent->speed = 105.0f;
					else if (VectorCompare(ent->s.angles, spawnvecs_stdm5[4]))
						ent->s.angles[0] = -95.0f;
				}
				else if (VectorCompare(ent->s.origin, spawnvecs_stdm5[5]))
					ent->classname = "item_armor_jacket_heavy";
				else if (VectorCompare(ent->s.origin, spawnvecs_stdm5[6])){
					ent->classname = "item_armor_helmet_heavy";
					ent->s.origin[1] = 480;	}
				else if (VectorCompare(ent->s.origin, spawnvecs_stdm5[7])){
					ent->classname = "item_armor_legs_heavy";
					ent->s.origin[1] = 480;	}
				else if (VectorCompare(ent->s.origin, spawnvecs_stdm5[8]))
					ent->classname = "item_armor_legs_heavy";
				else if (VectorCompare(ent->s.origin, spawnvecs_stdm5[9]))
					ent->classname = "item_armor_helmet_heavy";
				else if (VectorCompare(ent->s.origin, spawnvecs_stdm5[10]))
					ent->classname = "item_armor_helmet";
				else if (VectorCompare(ent->s.origin, spawnvecs_stdm5[11]))
					ent->classname = "item_armor_legs";
				//
				else if (VectorCompare(ent->s.origin, spawnvecs_stdm5[12]))
					ent->classname = "pistol_mod_rof";
				else if (VectorCompare(ent->s.origin, spawnvecs_stdm5[13]))
					ent->classname = "pistol_mod_rof";
				//items to health in air
				else if (VectorCompare(ent->s.origin, spawnvecs_stdm5[14])){
					ent->classname = "item_health_sm"; VectorSet(ent->s.origin, 640, -8, 576);	ent->dontDropToFloor = true;}
				else if (VectorCompare(ent->s.origin, spawnvecs_stdm5[15])){
					ent->classname = "item_health_sm"; VectorSet(ent->s.origin, 544, -16, 536);	ent->dontDropToFloor = true;}
				else if (VectorCompare(ent->s.origin, spawnvecs_stdm5[16])){
					ent->classname = "item_health_sm"; VectorSet(ent->s.origin, 960, -8, 576);	ent->dontDropToFloor = true;}
				else if (VectorCompare(ent->s.origin, spawnvecs_stdm5[17])){
					ent->classname = "item_health_sm"; VectorSet(ent->s.origin, 1048, -8, 536);	ent->dontDropToFloor = true;}
				//floating armour
				else if (VectorCompare(ent->s.origin, spawnvecs_stdm5[18])){
					VectorSet(ent->s.origin, 800, 8, 624);	ent->dontDropToFloor = true;}
				//	mega health			
				else if (VectorCompare(ent->s.origin, spawnvecs_stdm5[19])){
					ent->classname = "item_adrenaline"; VectorSet(ent->s.origin, 800, 512, 496); ent->dontDropToFloor = true;}
			}
...
"	yt	10 "
"	xr	-55 "
"	num 2 14" //hypov8 'frags' causing issue
"endif "
#endif
"talk "
//"hire "

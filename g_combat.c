...
void Killed (edict_t *targ, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point, int mdx_part, int mdx_subobject)
{
	...
	if (hit)
	{
		switch (meansOfDeath)
		{
			...
			//GUNRACE_START
			case MOD_M41A:
				attacker->client->resp.fav[8]++; //hypov8 fav[x]
				break;
			//todo UZI
			//todo sg3
			//todo sg4

			//hypov8 todo add new weps (delet fav from scoreboard)
			//GUNRACE_END
		}
	}
...
// do the damage
	if (take)
	{
		...
		//GUNRACE_START
		else if (mod == MOD_BLACKJACK)
		{
			if ((targ->svflags & SVF_MONSTER) || (client)){
				SpawnDamage(targ, TE_BLOOD, sparkspoint, normal, take); //TE_GIBS
				SpawnBloodPool(targ);
			}
			else

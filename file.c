...
int proccess_ini_file()
{
	...
	default_bonus[0]=0;
	default_weapon_order[0]=0; //add goat random
	...
	num_netnames = 0;
	num_ips = 0;
	fixed_gametype = true; //GUNRACE_ENABLE //hypov8 stop anyone seting teamplay
	...
	Com_sprintf (filename, sizeof(filename), "%s/gunrace.ini", dir); //GUNRACE_CHANGE
	...
	while (fgetline(infile, buffer))	// Retrieve line from the file
	{
		...
		else if (!strcmp(key, "weaponorder"))//add goat random
		{
			if (c==2) strncpy(weaponorder->string, param, 16);//add goat random
		}

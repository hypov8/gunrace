...
//===================================
//GUNRACE_START
	if (ent->movetype == MOVETYPE_NOCLIP) //hypov8 stop view movement
		return;
//GUNRACE_END
...
//		return;		// not in the player model
	...
	run *= 1 + (speed > 200);
	slide *= 1 + (speed > 200); //add hypov8 2 animations speeds for run(pistol strafe)

	if (client->pers.weapon /*&& (ent->noise_time > (level.time + 1.5))*/) //hypov8 noise effects the animation?
	{
	...
	//	slide = 0; //add hypov8 force fwd run animations, slide for pistol missing animation
	...
	if (client->anim_priority == ANIM_JUMP)
	{
		if (!ent->groundentity)
		{
			...
			tr = gi.trace( ent->s.origin, ent->mins, ent->maxs, endpos, ent, CONTENTS_LADDER | CONTENTS_SOLID);
...
//		if (ent->s.frame != FRAME_jump_07)
	...
	else if (run)	// running
	{
		...
		{
			if (run < 0) // standing run backward
			{
			...
			else // stand run forward
			{
				if (fabs(run) < 2)	// stood walking forwards
				{
					switch (weapontype)
					{
					case WEAPON_PISTOL:
				...
				else
				{
					...
					case WEAPON_PISTOL:
						if (client->buttons & BUTTON_ATTACK){
							...
							if (fabs(slide) < 2) {//walking strafe
								ent->s.frame = FRAME_walk_gdown_01;
								client->anim_end = FRAME_walk_gdown_10;
							}
							else{

...
void check_version(edict_t *ent)
{
	...
	sprintf(buf, "%s $gr_ver\n", ver_check);
	gi.WriteByte(13);
	gi.WriteString(buf);
	gi.unicast(ent, true);
	ent->version = 0; // MH: init version variable
}
//GUNRACE_END
...
#ifndef HYPODEBUG
	ent->flags -= FL_CHASECAM;//GUNRACE_ADD
#endif
}
// done.
...
void Cmd_Give_f (edict_t *ent)
{
	...
	if (give_all || Q_stricmp (gi.argv(1), "mods") == 0)
	{
		...
		ent->client->pers.hmg_shots = 300; //hypov8 was 30
...
// So I kept with this format for the menus that I added
	...
	if (!ent->client->pers.inventory[index])
	{
		...
		//	gi.dprintf ("silencer_shots: %d\n", ent->client->pers.silencer_shots);
			...
			{
				//cprintf (ent, PRINT_HIGH, "Out of item: %s\n", s); //GUNRACE_ADD //hypov8 disable nag when using wep keys
				return;
			}
			it = FindItem ("SPistol");
			index = ITEM_INDEX (it);
...
{
	//GUNRACE_START
	if (ent->client->showscores == SCORE_GUNRACE_STATS 
		|| (ent->client->pers.spectator == SPECTATING && 
		(level.modeset == MATCH || level.modeset == PUBLIC) && ent->client->showscores == NO_SCOREBOARD)) //hypo spec fixed
	{
		...
		gi.WriteString(va("%s\n", string));
		gi.unicast(ent, true);
	//ent->client->resp.scoreboard_hide = true;
...
void Cmd_CommandList_f (edict_t *ent)
{
	...
	if (ent->client->pers.admin > NOT_ADMIN && !fixed_gametype)
		cprintf(ent, PRINT_HIGH,
			...
			"      1 : Realmode\n"
			"      2 : Rocketmode\n"
			"   The teamplay settings are hardcoded to Standard DM\n");  //GUNRACE_ADD
		//	"      0 : Standard DM\n"
			//"      1 : Bagman\n"
		//	"      4 : Team DM\n");
}
...
void Cmd_ChangeMap_f (edict_t *ent)
{
	...
	safe_bprintf(PRINT_HIGH, "Admin changed map to %s\n", s); //hypov8 add
...
//GUNRACE_END
	...
	if (ent->client->pers.admin > NOT_ADMIN)
	{
		...
		if (i < 0) 
		{
			cprintf(ent, PRINT_HIGH, "The fraglimit can not be changed\n");
			//gi.cvar_set("fraglimit", value); //GUNRACE_DISABLED
			return;
		}
		//gi.cvar_set("fraglimit", value);//"0" //GUNRACE_DISABLED
		safe_bprintf(PRINT_HIGH, "the fraglimit can not be changed\n");
	}
	else
		cprintf(ent, PRINT_HIGH, "You do not have admin\n");
}
...
//GUNRACE_ADD
void Cmd_SetWeaponfrags_f(edict_t *ent, char *value)
{
	if (ent->client->pers.admin > NOT_ADMIN)
	{
		if (Q_stricmp(value, "") == 0)		{
			cprintf(ent, PRINT_HIGH, "Invalid value. current weaponfrags is %s\n", weaponfrags->string);
			return;
		}

		if (atoi(value) > 5)	value = "5";
		if (atoi(value) < 1)	value = "1";

		safe_bprintf(PRINT_HIGH, "weaponfrags set to %s\n", value);
		weaponfrags = gi.cvar_set("weaponfrags", value);
		cprintf(ent, PRINT_HIGH, "This setting will take effect with a map change\n");
	}
	else
		cprintf(ent, PRINT_HIGH, "You do not have admin, but weaponfrags is %s\n", weaponfrags->string); //change goat
}

void Cmd_SetWeaponorder_f(edict_t *ent, char *value) //add goat
{
	if (ent->client->pers.admin > NOT_ADMIN)
	{
		if (Q_stricmp(value, "") == 0){
			cprintf(ent, PRINT_HIGH, "Invalid value. current weaponorder is %s\n", weaponorder->string);
			return;
		}

		if (atoi(value) > 3)	value = "1";
		if (atoi(value) < 1)	value = "1";

		safe_bprintf(PRINT_HIGH, "weaponorder set to %s\n", value);
		weaponorder = gi.cvar_set("weaponorder", value);
		cprintf(ent, PRINT_HIGH, "This setting will take effect with a map change\n");
	}
	else
...
void Cmd_Rcon_f (edict_t *ent)
{
	...
	// ACEBOT_ADD
		...
		ACECM_BotAdd(gi.argv(2), gi.argv(3), gi.argv(4));
	else if (Q_stricmp(cmd, "removebot") == 0)// removebot
...
//GUNRACE_START //get version
	if (!Q_stricmp(cmd, ver_check)) 
	{
		...
		if (ent->version < GR_VER)
			safe_cprintf(ent, PRINT_CHAT, "Client files out of date.\nDownload at www.kingpin.info\n");
...
//GUNRACE_START
	else if (Q_stricmp (cmd, "setweaponfrags") == 0) //add hypov8
		Cmd_SetWeaponfrags_f(ent, gi.argv(1));
	else if (Q_stricmp (cmd, "setweaponorder") == 0) //add goat
		Cmd_SetWeaponorder_f(ent, gi.argv(1));
//GUNRACE_END
// ACEBOT_ADD
